...
/**
 * Asks user if they want to include the branch reference in the commit message
 * @param {string} branchRef - The branch reference to include
 * @returns {Promise<boolean>} True if user wants to include it
 */
export async function askIncludeBranchReference(branchRef) {
  const includeRef = await p.confirm({
    message: `Include branch reference (${branchRef}) in commit message?`,
    initialValue: false
  });

  if (p.isCancel(includeRef)) {
    p.cancel('Operation cancelled.');
    process.exit(0);
  }
